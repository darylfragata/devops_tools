...
| Status | Not Started / In Progress / Blocked / Done (dropdown, color-coded) |
| Due Date | If one exists |
| Notes | Anything relevant - blockers, context, links |
| Date Completed | Filled in when moved to Done |

## Workflow

1. New task assigned - add a row, fill ID/Task/Assigned By/Date Assigned/Priority, Status = Not Started.
2. Update Status as it moves (In Progress / Blocked), add Notes as needed.
3. Mark Done and fill Date Completed when finished.
4. Periodically move old Done rows out (e.g. to a dated archive tab, or delete) so the active view stays short.
